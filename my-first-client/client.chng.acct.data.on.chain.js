...
const program = Keypair.fromSecretKey(
  new Uint8Array([
    180, 137, 213, 5, 46, 163, 87, 113, 183, 38, 132, 196, 155, 103, 24, 159, 23, 185,
    182, 85, 52, 54, 197, 185, 156, 93, 207, 66, 186, 51, 253, 234, 58, 94, 185, 123,
    24, 200, 32, 92, 62, 189, 182, 246, 54, 186, 96, 186, 77, 83, 168, 220, 117, 90,
    121, 48, 74, 88, 216, 80, 101, 43, 124, 123
  ])
);
...
if (process.argv.length < 5) {
  ...
  console.log('2ND KEYPAIR WHOSE VALUE IS TO BE CHANGED ON-CHAIN.');
  console.log('3RD PARAM: (increment, decrement, reset) .\n\n');
  return;
}
...
let counterInstruction = process.argv[4];
console.log(counterInstruction.toString());
...
(async () => {
  ...
  let instruction = new TransactionInstruction({
    ...
    keys: [{ pubkey: acctKeypair.publicKey, isWritable: true }],
    data: Buffer.from(counterInstruction)
  })
